...
def get_user_by_token(token: str):
    return _db_users.find_one({"token": token})


def get_allowed_tokens():
    return list(filter(lambda x: x, (u.get("token", None) for u in _db_users.find({}))))
...
def update_or_create_user_by_token(token: str, **kwargs):
    ...
    _db_users.update_many({"user_token": token}, {"$set": {"token": token, **kwargs}}, upsert=True)
